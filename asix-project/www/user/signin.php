...
if ($_POST) {
    ...
    function encrypt_password($password) {
        // Aplica el cifrado SHA256 a la contraseña
        $hashed_password = hash('sha256', $password);
        return $hashed_password;
    }
    
    // Cifra la contraseña ingresada en el formulario de registro
    ...
    if ($result->num_rows > 0) {
        echo "<script> alert('This username is currently in use, please select another.'); </script>";
    } else {
        $insert = "INSERT INTO usuarios (nombre, pass) VALUES (?, ?)";
        $stmt = $conn->prepare($insert);
        $stmt->bind_param("ss", $user, $encrypted_password);
        if ($stmt->execute()) {
            echo "<script> alert('User correctly signed in.'); </script>";
        }
    }
...
<div class='dropdown'>
  ...
  ?>
    ...
    <div id="container">
        ...
        <center>
         <a style="text-align: center; color: black" href="login.php">Already have an account? Log in</a> 
    </div>
</body>
</html>